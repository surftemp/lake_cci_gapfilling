#!/bin/bash
# ===========================================================================
# Satellite CV Assembly — Parallel SLURM launcher
# ===========================================================================
#
# Submits two jobs:
...
#
# Usage:
#   bash run_satellite_cv_parallel.sh
#
# To change the experiment, edit RUN_ROOT below.
# ===========================================================================
...
ALPHA=a1000
MODE=physical
OUTPUT_DIR=${RUN_ROOT}/satellite_cv_assembly
...
if [ "${N_LAKES}" -eq 0 ]; then
    echo "ERROR: No lakes found. Check run root and completion status."
    exit 1
fi
...
#!/bin/bash
#SBATCH --job-name=sat_cv_extract
...
python -u ${SCRIPT} --run-root ${RUN_ROOT} --alpha ${ALPHA} --mode ${MODE} --output-dir ${OUTPUT_DIR} --phase extract --lake-id \${LAKE_ID}
...
python -u ${SCRIPT} --run-root ${RUN_ROOT} --alpha ${ALPHA} --mode ${MODE} --output-dir ${OUTPUT_DIR} --phase merge
